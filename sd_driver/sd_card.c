...
static uint8_t sd_write_block(sd_card_t *pSD, const uint8_t *buffer,
    ...
    // write the data
    sd_spi_transfer(pSD, buffer, NULL, length);
...
static int in_sd_write_blocks(sd_card_t *pSD, const uint8_t *buffer,
    ...
    if (blockCnt == 1) {
        ...
        if (SD_BLOCK_DEVICE_ERROR_NONE != (status = sd_cmd(pSD, CMD25_WRITE_MULTIPLE_BLOCK, addr, false, 0))) {
            return status;
        }
...
bool sd_init_driver() {
    ...
    if (!initialized) {
	    ...
	    */
	    gpio_set_drive_strength(pSD->ss_gpio, sd_gpio_drive_strength);

            // Chip select is active-low, so we'll initialise it to a
            // driven-high state.
	    //sd_spi_init_pl022(pSD);
...
inline sd_card_t *sd_get_by_num(size_t num) {
	return &sd_cards[0];
}
