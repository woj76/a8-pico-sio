...
An Atari 8-bit SIO device implementation for a [Raspberry Pi Pico (2)](https://www.raspberrypi.com/products/raspberry-pi-pico/)

(Note: this is still a draft under construction and has not been double checked.)
...
This project is an Atari Drive SIO emulator implemented on a Raspberry PI Pico, similar to SDrive-MAX or SIO2SD. The first main motivation was to support loading of all kinds of turbo CAS tape images (not only the plain Atari SIO tape images, but all kinds that require the tape recorder hardware expansions like K.S.O. Turbo 2000, Blizzard, etc.) on a real Atari. (So far the only actual device capable of that seems to be the AVG/SUBCart, but only through the SIO port's TX/RX lines, or the so far unpublished Bluetooth audio devices by Zaxxon, but these are mostly for WAV files.) Another driving force was to get acquainted with the Pico platform and the SIO protocol, hence a proper Dn: "classic" drive emulation was added too, with write and ATX support. Finally, I opted for a no driver program on the Atari side approach, but a display based UI on the Pico side, and then also for a non-touch screen solution (I have been off-put by constant screen calibration problems on my SDrive-MAX). 
...
* Ultra Speed SIO with Pokey divisors (hex) 10, 6, 5, 4, 3, 2, 1, and 0 (always inactive for ATX images).
* Use of both cores on the Pico which enables fully concurrent operation of the Atari communication (SIO/tape playing) with the GUI and file selection with no user hold back.
* Current configuration saving to FLASH and remembering the last selected directory on power down.

What is **not (yet in some cases) supported**:

* MAP files (there is always access to the GUI for selecting new images and the rotate buttons though)
...
* The Pimoroni Display Pack 2.0 for Pico: https://shop.pimoroni.com/products/pico-display-pack-2-0?variant=39374122582099
...
The carrier board relives you of the wires, disconnecting the 4 wires to enable the SD card reader, integrates the SD card slot on the board, provides the pull-down resistor on the SIO motor line for the Pico 2, integrates the UART TX Shottky diode, and finally provides the facility to power the device from Atari through a MODFET that disconnects the Atari power upon connecting the USB cable. 

**Note on powering the Pico SIO device from the Atari:** From the figures I have seen (in the SDrive-MAX documentation, not sure what the actual source of this information is), the +5/Ready line on the SIO port is rated at max 50mA, I measured the complete Pico SIO device to use upwards of **70mA**. It seemed to be fine with no other devices on the SIO chain, but I know I had problems powering the SDrive-MAX that uses ~150mA with my Ataris, especially the 800XL. So, in case of any problems, just use the USB port on the Pico to power it.

The Gerber files for the board are included in the project, you can have the board produced at JLCPCB or PCBWay and solder the handful of elements to it:

* For the SD card slot integration you need the micor-SD card socket, the two capacitors C1 and C2, and resistors R1, R2, R3, R4, and R6.
* If you do not care about other SIO devices on the chain and/or you are short on SMD Shottky diodes, you can solder in a 0Ω 0805 resistor in place of D1 (the pads are made large enough for that), or bridge the connection with a wire / solder blob.
* The R7 can in theory also be bridged instead, but it is much better and safer to use a 1kΩ resistor there (size 0603).
* The R5 resistor is needed unconditionally for Pico 2 if any of the functionality requiring the motor line is needed.
* The RESET button is entirely optional, but makes restarting of the device or uploading the firmware during development much easier.
...
* R1 to R6: 4.7kΩ 0603 resistors (R5 is only needed for Pico 2, see above, but can be there for other Picos if you want to have universal all Pico friendly board).
...
* <xxx> for standalone Pico 1 board operation (1MB of internal storage)
* <xxx> for Pico 1 on the carrier board (same storage)
* <xxx> for standalone Pico 2 board operation (3MB of internal storage)
* <xxx> for Pico 2 on the carrier board (same storage)
* <xxx> for standalone [Pimoroni Pico Lipo 16MB](https://shop.pimoroni.com/products/pimoroni-pico-lipo) (15MB of internal storage)
* <xxx> for Pimoroni Pico Lipo 16MB on the carrier board (same storage)
...
The PAL/NTSC options toggles between PAL or NTSC "friendly" baud rates keeping them as close as possible to what the Pokey expects on a particular system. This happens on the next SIO speed change (get speed command from the Atari or high speed divisor setting change).
...
The XEX loader option relocates the virtual disk boot loader to the specified address, this is effective on the next XEX file mount or re-mount.

The Turbo data and activation pin condition options can be used to configure for the particular turbo emulation setup. For typical known turbo systems use the following:
...
* Data on joystick port 2 pin 1, no special activation (motor line) for Turbo D
...
However, the device will work with any combination of these as long as the Atari business end can understand them. This option becomes active on the next CAS image file mount or re-mount/rewind. Note also that some turbo systems are sensitive to the PWM signal polarity more than others and depending on how the CAS file was produced the PWM Invert option might need to be turned on too.
...
You can umount the slots while they are being read by the Atari, in which case the corresponding image transfer will of course fail. When the SD card is removed and there are any mounts referring to the files on the SD cards, they will be fully emptied. 
...
Finally, upon file selection for mounting a disk drive you can choose to create a new disk image in the current directory, either empty or pre-formatted for the most common Atari DOS-es (2.0/2.5, MyDOS, and SpartaDOSX).

### LED and Screen Indicators

When operating / interacting with the Atari, the color LED on the display will flash. Green flashes indicate a read transfer from the current device, red indicates a write transfer to the current device/disk, and in addition to that the blue color can be flashing, which indicates high speed SIO operation for the drives, or a turbo/PWM operation for the C: device. The current (or most recent) device is indicated with a green circle on the display on the left of the drive letter, the red cross indicates that a drive has been unmounted by the user, the mounted file stays attached to the drive to enable quick remount (see above), unless the hosting media (SD card) is removed, in which case all the entries referring to the removed media are emptied. 
...
* **DjayBee** for providing the ATX disk image suite,
...
* **drac030** for a handful of vital comments.
